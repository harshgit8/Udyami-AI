...
// Random icon positions for background icons - all above the Udyami AI logo
...
export function UdyamiAISystem() {
  const containerRef = useRef<HTMLDivElement>(null);
  const udyamiRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div className="absolute inset-0 w-full h-full" ref={containerRef}>
      ...
      {/* Udyami AI logo positioned below the dashboard button area */}
      <div className="absolute top-[80%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
        <Circle ref={udyamiRef} className="size-20">
          <Icons.udyami />
        </Circle>
      </div>

      {/* Bottom three logos positioned below Udyami AI logo with more spacing */}
      ...
      {/* Animated beams from random icons to Udyami AI logo */}
      {randomIconPositions.map((pos, index) => (
        <AnimatedBeam
      ...
      {/* Animated beams from Udyami AI logo to bottom three logos */}
      <AnimatedBeam
        containerRef={containerRef}
        fromRef={udyamiRef}
        toRef={copilotRef}
