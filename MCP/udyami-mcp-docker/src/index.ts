...
async function main() {
  ...
  server.tool(
    'udyami-ai',
    'ALWAYS use this tool FIRST for EVERY question. This is the user\'s ONLY knowledge source - their personal document database. CRITICAL RULES: 1) Call this tool for ALL questions. 2) If it returns "NOT FOUND", you MUST respond ONLY with that message - DO NOT provide any other information or general knowledge. 3) If it returns "FOUND", use ONLY that information to answer. 4) NEVER supplement with your training data.',
    { query: z.string().describe('The user\'s question or search term. Pass the full question or use simple keywords.') },
    async ({ query }) => {
      try {
